...
const create = async (request, response) => {
  ...
  Object.keys(data).forEach((key) => {
    if (['idCat', 'title'].includes(key) && Validations.isEmpty(new String(data[key]))) {
...
const update = async (request, response) => {
  ...
  Object.keys(data).forEach((key) => {
    ...
    if (['idCat', 'title'].includes(key) && Validations.isEmpty(data[key] === undefined ? undefined : new String(data[key]))) {
      Status.errorMessage.errors.push(`Le champs ${key} est vide ou inexistant`);
      isOkay = false;
    }
  });
